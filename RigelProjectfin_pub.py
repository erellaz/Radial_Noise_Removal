...
# Visit http://erellaz.com for details
# 2017-03-05
#______________________________________________________________________________
#Parameters, change only this block:
#input fit file to process
fitin='C:\Users\User\Desktop\WH3.fit'
fitout='C:\Users\User\Desktop\WH3_radon.fit'
...
for channel in range(0,scidata.shape[0]):#loop on all the channels 1 (mono) or 3(RGB)   
    ...
    for ix in range(0,splitx):
        for iy in range(0,splity):
            ...
            cx=sx*(ix+.5)
            cy=sy*(iy+.5)          
            # Calculating the angle from the source
            radiance=180*math.atan2(ry-cy,rx-cx)/math.pi
            if radiance<0:
                radiance=radiance+180
            #Radon Filter the chunk            
            imf=radfil(im,radiance,arange)
